...
from flax.training import checkpoints # to save checkpoints
import jax
import ml_collections
import numpy as np
import tensorflow as tf

from label_dp.label_dp import datasets
from label_dp.label_dp import models
from label_dp.label_dp import utils
...
def report_metrics(i_stage, report_name, epoch, metrics, writer):
  ...
  summary = jax.tree_util.tree_map(lambda x: float(x.mean()), metrics)
...
def multi_stage_train(configs: ml_collections.ConfigDict, workdir: str):
  ...
  use_vit = 'vit' in configs.model.arch.lower()
  print("use vit: ", use_vit)
  orig_dataset = datasets.TFDSNumpyDataset(
      name=configs.data.name, **configs.data.kwargs, resize_to_224=use_vit)
  ...
  for i_stage in range(len(stage_datasets)):
    if configs.stage_specs[i_stage]['type'] == 'rr':
      k_for_prior = compute_randomized_labels(
          stage_datasets[i_stage], configs.train.batch_size,
      ...
      k_for_prior = compute_randomized_labels_with_priors(
          stage_datasets[i_stage], model, last_stage_state, configs.train.batch_size,
          configs.stage_specs[i_stage], n_tr_total)
    else:
    ...
    if i_stage != 0 and configs.reuse_last_stage_data:
      ...
      if configs.mask_last_stage_label_by_prior:
        ...
        filter_stage_data_by_prior(
            ...
            n_tr_total, configs.train.batch_size)
...
def single_stage_train(
  ...
  rng = jax.random.PRNGKey(configs.train.run_seed + i_stage)
  np_rng = np.random.RandomState(seed=configs.train.run_seed + i_stage + 123)
  ...
  local_batch_size = utils.get_local_batch_size(configs.train.batch_size)
  dataset = stage_datasets[i_stage]

  n_train = dataset.get_num_examples('train')
  n_train_steps = int(configs.train.num_epochs * n_train / local_batch_size)
  lr_fn = utils.build_lr_fn(
      configs.lr_fn.name, configs.optimizer.learning_rate, n_train_steps, configs.lr_fn.kwargs)
  ...
  p_train_step = jax.pmap(
      functools.partial(utils.train_step, model.apply, l2_regu=configs.train.l2_regu),
      axis_name='batch')
  ...
  with metric_writers.ensure_flushes(writer):
    while True:
      ...
      if epoch >= configs.train.num_epochs:
        break
      ...
      # === save a checkpoint ===
      if jax.process_index() == 0:              # only save once in multi‐host setups
        state_single = jax_utils.unreplicate(state)
        import os
        abs_ckpt_dir = os.path.abspath(os.path.join(workdir, f'stage{i_stage}'))
        tf.io.gfile.makedirs(abs_ckpt_dir)

        epoch_scalar = int(state_single.epoch)          # now a Python int
...
def compute_randomized_labels(dataset, batch_size, spec, n_tr_total):
  ...
  dataset.subset_mask = np.ones(len(dataset.subset_index), dtype=bool)
...
def filter_stage_data_by_prior(last_dataset, model, last_stage_state,
  ...
  model_vars = {'params': last_stage_state.params,
                **last_stage_state.model_states}
  j_pred = jax.jit(functools.partial(model.apply, train=False))
  global_mask = np.ones(n_tr_total, dtype=bool)
