...
class TFDSNumpyDataset:
  ...
  def __init__(self, name, random_crop=True, random_fliplr=True,
               random_cutout=0, resize_to_224=False):
    ...
    self._resize_to_224 = resize_to_224
  ...
  def get_input_shape(self, input_name):
    if input_name == 'image':
      original = self.ds_np['train']['image'].shape[1:]
      if self._resize_to_224:
      # Return 224x224 shape for ViT models
        return (224, 224, original[2])  # Keep original number of channels
      else:
        return original
  ...
  def normalize_images(self, batch_image_np):
    ...
    if self._resize_to_224:
      ...
      # Convert to tf tensor, resize, then back to numpy
      images_tf = tf.convert_to_tensor(images)
      images_tf = tf.image.resize(images_tf, [224, 224])
...
class LabelRemappedTrainDataset:
  ...
  def __init__(self, dataset, subset_index):
    ...
    self.subset_mask = np.ones(len(subset_index), dtype=bool)
